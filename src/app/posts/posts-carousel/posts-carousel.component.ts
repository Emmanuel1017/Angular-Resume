...
export class PostsCarouselComponent implements OnInit {
    ...
    @Output() postOpened = new EventEmitter<IPost>();
    ...
    set currentPage(value: number) {
        if (value) { this._currentPage = value; this.populateCarousel(); }
    }

    @Input() get posts(): IPost[] { return this._posts; }
    set posts(value: IPost[]) {
        if (value) { this._originalPosts = value; this.onResizeElement(); }
    }

    ngOnInit(): void {}

    hasContent(post: IPost): boolean {
        return !!(post.internationalizations?.find(i => i.language === 'en')?.content);
    }
